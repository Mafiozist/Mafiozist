...
| --- | --- | --- |
| 2026-07-17 | Фундамент **синхронизации (v0.3)**: движок обмена снапшотами с LWW-слиянием (`export_snapshot`/`apply_snapshot`), абстракция `SyncTransport` (реализации `FileTransport` — файл, в т.ч. в папке Яндекс.Диска; `InMemoryTransport` — тесты); `sync()` двусторонний (скачать→слить→выгрузить). `SystemClock` переведён на секундную точность (корректность строкового LWW). IPC-команда `sync_file` + кнопка «Синхронизация» в UI. Ядро **24 теста** (+2: конвергенция двух устройств, LWW). REST/OAuth-адаптер Яндекса — задокументированный seam на будущее. | opus |
| 2026-07-17 | UI-доводка: **удаление проектов и серий** с подтверждением (каскад через ядро), **экспорт серии в Markdown** (скачивание файла из блоков). Сборка/тесты зелёные. | opus |
| 2026-07-17 | Редактор блоков доведён: **редактирование на месте** (тип/заголовок/текст → Сохранить/Отмена, `update_content`) и **drag-and-drop сортировка** блоков через `@dnd-kit` (оптимистично + `reorder_content`). Сборка/тесты зелёные. | opus |
